...
class _MyCustomFormState extends State<WeatherPages> {
  ...
  Future<List<List<Object?>>> fetchCitySuggestions(String pattern) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to load city suggestions'); // Handle error if necessary
      }
    } catch (e) {
      throw Exception('City not found'); // Handle error if necessary
    }
  }
  ...
  Future<WeatherAPI?> fetchWeather(double latitude, double longitude) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to load weather data from API'); // Handle error if necessary  
    }
  }
    ...
    void printGeoLocation() async {
      ...
      setState(() {
        ...
        for (int i = 0; i < 24; i++) {
          ...
          String temperature2m =
              '${currentlyTabAPI.hourly!.temperature2m![i].toString()}°C';
